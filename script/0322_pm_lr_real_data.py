...
CUDA_VISIBLE_DEVICES=1 python3 script/0322_pm_lr_real_data.py
"""
# This is a simper version that do not display the traditional results for some reasons
import argparse
import sys
import os
import glob
...
# 1. Plotting Utility
# ==========================================
def plot_comparison_5xn(inputs, dl_recons, slice_nums, save_path, slice_gap=4):
    """
    Creates a 5-column grid. 
    For each group of 5 slices, there are 2 rows:
      - Row 1: Low Res Inputs
      - Row 2: DL Reconstructions
    """
    # Apply slice gap
    ...
    total_rows = row_groups * 2  # 2 rows per group (Input, DL)
    ...
    for i in range(num_slices):
        ...
        r_in = r_group * 2          # Row 1: Input
        r_dl = r_group * 2 + 1      # Row 2: DL Recon
    ...
    for i in range(num_slices, row_groups * cols):
        c = i % cols
        r_group = i // cols
        axes[r_group * 2, c].axis('off')
        axes[r_group * 2 + 1, c].axis('off')
...
if __name__ == "__main__":
    ...
    with torch.no_grad(): 
        # Note: real_trad is still unpacked here to avoid changing the dataset class, 
        # but it is completely ignored in the plotting/collection logic below.
    ...
    # Generate the unified 5*N grid comparison plot without traditional recons
    plot_path = os.path.join(opt.log_dir, "Inference_Comparison_Grid_5xN.png")
    plot_comparison_5xn(collected_inputs, collected_dl_recons, collected_slice_nums, plot_path, slice_gap=4)