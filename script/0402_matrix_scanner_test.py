...
CUDA_VISIBLE_DEVICES=3 python3 script/0402_matrix_scanner_test.py
"""
...
def plot_comparison_5xn(inputs_trad, dl_recons, slice_nums, save_path, slice_gap=4):
    ...
    # Changed to 2 rows per group (Trad input, DL output)
    total_rows = row_groups * 2  
    ...
    for i in range(num_slices):
        c = i % cols
        r_group = i // cols
        r_trad, r_dl = r_group * 2, r_group * 2 + 1
        
        axes[r_trad, c].imshow(inputs_trad[i], cmap='gray')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_dir', type=str, default=f'log/{time_prefix}_matrix_model_scanner_test')
