...
CUDA_VISIBLE_DEVICES=3 python3 script/0403_unrolling_test.py
"""
...
# ==========================================
# 2. Plotting Utility (Now with Error Maps)
# ==========================================
def plot_comparison_5xn(inputs_trad, dl_recons, slice_nums, save_path, slice_gap=4):
    inputs_trad = inputs_trad[::slice_gap]
    dl_recons = dl_recons[::slice_gap]
    slice_nums = slice_nums[::slice_gap]
    
    num_slices = len(inputs_trad)
    if num_slices == 0: return
        
    cols = 5
    row_groups = math.ceil(num_slices / cols)
    # 3 rows per group: Trad input, DL output, and Error Map
    ...
    for i in range(num_slices):
        c = i % cols
        r_group = i // cols
        r_trad, r_dl, r_err = r_group * 3, r_group * 3 + 1, r_group * 3 + 2
        
        # Calculate absolute difference
        err_map = np.abs(inputs_trad[i] - dl_recons[i])
        
        # Plotting
        axes[r_trad, c].imshow(inputs_trad[i], cmap='gray', vmin=0, vmax=1)
        axes[r_dl, c].imshow(dl_recons[i], cmap='gray', vmin=0, vmax=1)
        axes[r_err, c].imshow(err_map, cmap='hot', vmin=0) # 'hot' highlights differences beautifully
        
        # Text Labels
        axes[r_trad, c].text(0.05, 0.95, f"{slice_nums[i]} (Trad)", color='cyan', 
                             fontsize=10, fontweight='bold', transform=axes[r_trad, c].transAxes,
                             va='top', ha='left')
        axes[r_dl, c].text(0.05, 0.95, "DL Output", color='yellow', 
                           fontsize=10, fontweight='bold', transform=axes[r_dl, c].transAxes,
                           va='top', ha='left')
        axes[r_err, c].text(0.05, 0.95, "Error Map", color='white', 
                           fontsize=10, fontweight='bold', transform=axes[r_err, c].transAxes,
                           va='top', ha='left')
...
class SPENScannerDataset(Dataset):
    def __init__(self, data_dir, trad_key='Image_SPEN'):
    ...
    def __getitem__(self, idx):
        ...
        # Process Trad Image_SPEN (This is the input to the network)
        img_trad = np.abs(np.squeeze(mat_data[self.trad_key]))
        img_trad = (img_trad - img_trad.min()) / (img_trad.max() - img_trad.min() + 1e-8)
        
        return torch.FloatTensor(img_trad).unsqueeze(0), Path(f_path).stem
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ckpt', type=str, default='/home/data1/musong/workspace/python/spen_recons/log/04020855_spen_matrix_recons/ckpt/best.pth')
    parser.add_argument('--log_dir', type=str, default=f'log/{time_prefix}_adam_unrolling_test')
    ...
    all_trad, all_dl, all_ids = [], [], []

    print(f"[*] Testing {len(dataloader)} slices...")
    with torch.no_grad():
        for i, (img_trad, file_id) in enumerate(dataloader):
            # The traditional MATLAB reconstruction IS the input
            output = model(img_trad.to(device))
    ...
    plot_comparison_5xn(all_trad, all_dl, all_ids, os.path.join(opt.log_dir, "Scanner_Comparison_Grid.png"))
