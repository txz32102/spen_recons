...
CUDA_VISIBLE_DEVICES=1 python3 script/0417_supervised_train_v2.py
"""
...
class PhaseMapNet(nn.Module):
    def __init__(self, in_channels: int = 2, base_channels: int = 96, num_blocks: int = 8) -> None:
        ...
        layers.extend(
            [
                nn.Conv2d(base_channels, base_channels, kernel_size=3, padding=1),
                nn.LeakyReLU(0.2, inplace=True),
                # The final convolution layer directly outputs the 1-channel phase map
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Directly output the phase map without the torch.pi * torch.tanh() post-process
        return self.net(x)
...
class SupervisedPhaseReconModel(nn.Module):
    ...
    def forward(self, ghosted_data: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        mag = torch.sqrt(features[:, 0:1]**2 + features[:, 1:2]**2 + 1e-8)
        max_mag = torch.amax(mag, dim=(-2, -1), keepdim=True)
        features = features / (max_mag + 1e-8)
...
def save_visualization(
    ...
    # Metrics against Ground Truth (gt_display)
    ghosted_psnr_gt, ghosted_ssim_gt = compute_image_metrics(ghosted_display, gt_display)
    inva_psnr_gt, inva_ssim_gt = compute_image_metrics(inva_display, gt_display)
    corrected_psnr_gt, corrected_ssim_gt = compute_image_metrics(corrected_display, gt_display)
    good_psnr_gt, good_ssim_gt = compute_image_metrics(good_display, gt_display)

    # Metrics against GT-Corrected Recon (good_display)
    ghosted_psnr_good, ghosted_ssim_good = compute_image_metrics(ghosted_display, good_display)
    inva_psnr_good, inva_ssim_good = compute_image_metrics(inva_display, good_display)
    corrected_psnr_good, corrected_ssim_good = compute_image_metrics(corrected_display, good_display)
    ...
    image_rows = [
        (ghosted_display.detach().cpu().numpy(), "Ghosted", ghosted_psnr_gt, ghosted_ssim_gt, ghosted_psnr_good, ghosted_ssim_good),
        (inva_display.detach().cpu().numpy(), "InvA Recon", inva_psnr_gt, inva_ssim_gt, inva_psnr_good, inva_ssim_good),
        (corrected_display.detach().cpu().numpy(), "Corrected Recon", corrected_psnr_gt, corrected_ssim_gt, corrected_psnr_good, corrected_ssim_good),
        # For the Good Recon panel, we only show its metrics against the GT (comparing Good to Good is redundant)
        (good_display.detach().cpu().numpy(), "GT-Corrected Recon", good_psnr_gt, good_ssim_gt, [None] * cols, [None] * cols),
        (gt_display.detach().cpu().numpy(), "Ground Truth", [None] * cols, [None] * cols, [None] * cols, [None] * cols),
    ]

    for col in range(cols):
        for row, (images, title, psnr_gt, ssim_gt, psnr_good, ssim_good) in enumerate(image_rows):
            axes[row, col].imshow(images[col], cmap="gray", vmin=0, vmax=1)
            
            metric_lines = []
            if psnr_gt[col] is not None:
                metric_lines.append(f"vs GT: PSNR {psnr_gt[col]:.2f} / SSIM {ssim_gt[col]:.3f}")
            if psnr_good[col] is not None:
                metric_lines.append(f"vs Good: PSNR {psnr_good[col]:.2f} / SSIM {ssim_good[col]:.3f}")
...
def setup_logger(log_dir: str) -> logging.Logger:
    logger = logging.getLogger("supervised_phase_0417_v2")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--exp_name", type=str, default="supervised_phase_0417_v2")
