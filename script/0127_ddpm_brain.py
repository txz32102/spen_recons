...
accelerate launch script/0127_ddpm_brain.py \
  --train_data_dir="data/0127_fastmri_brain_384" \
  --resolution=256 \
  --center_crop \
  --random_flip \
  --output_dir="log/01270011_ddpm_fastmri_brain_256" \
  --train_batch_size=8 \
  --num_epochs=100 \
    ...
    def transform_images(examples):
        ...
        for image in examples["image"]:
            if not args.preserve_input_precision:
                processed.append(augmentations(image.convert("L")))
            else:
                precise_image = image
                if precise_image.mode == "P":
                    precise_image = precise_image.convert("L")
